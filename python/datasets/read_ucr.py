...
def labeledTsListFromDataset(datasetDir, useTrain=True, useTest=True,
	instancesPerTs=10, minPaddingFraction=1.25, **paddingKwargs):
	...
	tsList = concatedTsList(X, Y, instancesPerTs=instancesPerTs,
		...
		minPaddingFractionOfLength=minPaddingFraction, **paddingKwargs)
...
def getShortUCRDatasetDirs():
	return map(dirFromName, SHORT_UCR_DATASETS)
...
if __name__ == '__main__':
	...
	if plotTs:
		...
		# for dataDir in getAllUCRDatasetDirs():
		for dataDir in getShortUCRDatasetDirs():
			tsFromDataset = labeledTsListFromDataset(dataDir,
					instancesPerTs=howManyPerTs)[:howManyPerDataset]
	...
	if plotSubseqs:
		...
		for dataDir in getShortUCRDatasetDirs():
			tsFromDataset = labeledTsListFromDataset(dataDir,
					instancesPerTs=howManyPerDataset)
			saveSubdir = os.path.join(saveDir, nameFromDir(dataDir))
			for ts in tsFromDataset:
				print ts.name, ts.data.shape
