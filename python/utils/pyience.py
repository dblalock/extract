...
class Options(object):
	...
	def _raise(self):
		raise TypeError("Options object is not a collection; use"
			"options.values to access the collection of individual options")
		...
		if _is_kv_arg(arg): # it's a keyword argument
			...
			if arg_num > num_positional_keys:
				raise UsageError("only expecting "
					"{} positional arguments!".format(num_positional_keys))
...
def generate_save_path(params, savedir, subdir_keys=None):
	...
	# 	if not hasattr(subdir_keys, '__len__'):
	# 		subdir_keys = [subdir_keys]
	# 	if len(subdir_keys):
	# 		create_subdirs = True
	...
	if fractions is not None and len(fractions):
		if len(fractions) != n_folds:
			raise ValueError("Specified fractions of total for {} groups, but "
				"n_fold is {}; ignoring n_fold".format(len(fractions), n_fold))

	if stratified:
		all_idxs = [[] for i in range(n_folds)]
		lbl2idxs = uniqueElementPositions(labels)
		for lbl, idxs in lbl2idxs.items():
			if len(idxs) < n_folds:
				warnings.warn(("Label {} appears only {} times, which is "
								"less than the number of folds requested, {}".format(
									lbl, len(idxs), n_folds)), Warning)
